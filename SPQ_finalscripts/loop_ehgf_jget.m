...
input = load([data_path, 'forwardinput.txt']); % same for all subjects, only load once
score = load([data_path,'trialscoreL3_6.txt']); % different for subjects
    ...
    u4(:,1) = input(41:80);
    u4(:,2) = [repmat(mean(u4(1:10,1,1)),10,1,1); repmat(mean(u4(11:20,1,1)),10,1,1); repmat(mean(u4(21:28,1,1)),8,1,1); repmat(mean(u4(29:39,1,1)),11,1,1); u4(end,1,1)];
    u4(:,3) = [repmat(std(u4(1:10,1,1)),10,1,1); repmat(std(u4(11:20,1,1)),10,1,1); repmat(std(u4(21:28,1,1)),8,1,1); repmat(std(u4(29:39,1,1)),11,1,1); u4(end,1,1)];
    
    u5(:,1) = input(81:164);
    u5(:,2) = [repmat(mean(u5(1:10,1,1)),10,1,1); repmat(mean(u5(11:21,1,1)),11,1,1); repmat(mean(u5(22:31,1,1)),10,1,1); ...
              repmat(mean(u5(32:42,1,1)),11,1,1); repmat(mean(u5(43:52,1,1)),10,1,1);  repmat(mean(u5(53:62,1,1)), 10,1,1); ...
              repmat(mean(u5(64:74,1,1)),11,1,1); repmat(mean(u5(74:end,1,1)),11,1,1)];
    u5(:,3)  = [repmat(std(u5(1:10,1,1)),10,1,1); repmat(std(u5(11:21,1,1)),11,1,1); repmat(std(u5(22:31,1,1)),10,1,1); ...
              repmat(std(u5(32:42,1,1)),11,1,1); repmat(std(u5(43:52,1,1)),10,1,1);  repmat(std(u5(53:62,1,1)), 10,1,1); ...
              repmat(std(u5(64:74,1,1)),11,1,1); repmat(std(u5(74:end,1,1)),11,1,1)];
     
    u6(:,1)=input(165:206); % @Toni, can you adapt this for level 6 as in the levels above? 
    u6(:,2) = [repmat(mean(u6(1:10,1,1)),10,1,1); repmat(mean(u6(11:31,1,1)),21,1,1); repmat(mean(u6(32:42,1,1)),11,1,1)];
    u6(:,3)  = [repmat(std(u6(1:21,1,1)),21,1,1); repmat(std(u6(22:42,1,1)),21,1,1)];
    
    %u7(:,1) = input(1:206); 
    %u7(:,2) = [u3(:,2); u4(:,2); u5(:,2), u6(:,2)];
    %u7(:,3) = [u3(:,3); u4(:,3); u5(:,3), u6(:,3)];
...
%Response(5).Position = {load(fullfile(data_path, strcat('forwardresponse_wholegame', '.txt')))}; 
%Response(5).Confidence = {load(fullfile(data_path, strcat('forwardresponseBEAM_wholegame', '.txt')))}; 
...
% Create string matrix of perception and response model combinations
          jGET_inputs = ["tapas_ehgf_jget_Level3_config",  "tapas_gaussian_obs_spacetask_config";
                         ...
                         %"tapas_ehgf_jget_OverallGame_config", "tapas_gaussian_obs_spacetask_config";]
...
idx = 0; 
for level = 5 % goes through levels
...
%                         tapas_hgf_jget_spacetask_plotTraj(est(sub));
                ...
                % Save models
                save(strcat("est_withnb_level_", num2str(level), '.mat'),'est','-mat');
                clear u
                %             end
        end
%         
